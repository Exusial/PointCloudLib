...
class ThreeNN(jt.Function):
    ...
    @alias(dist, out0)
    @alias(idx, out1)
    three_nn_kernel_launcher_fast(unknown_shape0, unknown_shape1, known_shape1, unknown_p, known_p, dist_p, idx);
    '''
    def execute(ctx, unknown, known):
        ...
        cuda_header=cuda_header, cuda_src=forward_src)
...
class ThreeInterpolate(jt.Function):
    ...
    def execute(self, features, idx, weight):
        ...
        self.weight = weight 
        self.m = m
        output = jt.code([B,c,n],feature.dtype,[features,idx,weight],cuda_header=cuda_header,cuda_src=forward_src)
        return output
    ...
    def grad(ctx, grad_out):
        ...
        B, c, n = grad_out.size()
        grad_features = jt.code([B,c,m],grad_out.dtype,[grad_out, idx, weight], cuda_header=cuda_header,cuda_src=backward_src)
