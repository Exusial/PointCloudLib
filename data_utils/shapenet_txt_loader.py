...
class ShapeNetPartNormal(Dataset):
    ...
    cls2parts = []
    cls2partembed = jt.zeros((16, 50))
    for i, cls in enumerate(classes):
        idx = cls_parts[cls]
        cls2parts.append(idx)
        cls2partembed[i].scatter_(0, jt.array(idx), jt.ones((50)))
    ...
    def __init__(self,
        ...
        if transform is not None:
            self.transform = [get_transform(trans) for trans in transform]
        else:
            self.transform = None
        if presample and not os.path.exists(filename):
            ...
            self.data, self.cls = [], []
            npoints = []
            fps = FurthestPointSampler(num_points)
            for cat, filepath in tqdm(self.datapath, desc=f'Sample ShapeNetPart {split} split'):
                ...
                data = jt.array(data).unsqueeze(0)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            for trans in self.transform:
                data = trans(data)
        pdata = jt.concat([data[key] for key in ['pos', 'x', 'heights']], -1)
        return pdata, cls, seg
