...
def get_aggregation_feautres(p, dp, f, fj, feature_type='dp_fj'):
    if feature_type == 'dp_fj':
        fj = jt.concat([dp, fj], 1)
    elif feature_type == 'dp_fj_df':
        df = fj - f.unsqueeze(-1)
        fj = jt.concat([dp, fj, df], 1)
    elif feature_type == 'pi_dp_fj_df':
        df = fj - f.unsqueeze(-1)
        fj = jt.concat([p.transpose(1, 2).unsqueeze(-1).expand(-1, -1, -1, df.shape[-1]), dp, fj, df], 1)
    elif feature_type == 'dp_df':
        df = fj - f.unsqueeze(-1)
        fj = jt.concat([dp, df], 1)
    return fj
...
class FeaturePropogation(nn.Module):
    ...
    def execute(self, pf1, pf2=None):
        ...
        if pf2 is None:
            ...
            if f1 is not None:
                f = self.convs(
                    jt.concat((f1, three_interpolation(p1, p2, f2)), dim=1))
            else:
...
class PointNextPartDecoder(nn.Module):
    ...
    def execute(self, p, f, cls_label):
        ...
        if self.cls_map == 'curvenet':
            ...
            emb1 = emb1.max(dim=-1, keepdims=True)  # bs, 64, 1
            emb2 = self.global_conv2(f[-1])
            emb2 = emb2.max(dim=-1, keepdims=True)  # bs, 128, 1
            cls_one_hot = jt.zeros((B, self.num_classes))
            cls_one_hot = cls_one_hot.scatter_(1, cls_label, jt.ones((B, self.num_classes))).unsqueeze(-1)
        ...
        decoder_sequece = nn.Sequential([self.decoder[0][k] for k in range(1, len(self.decoder[0]))])
        f[-len(self.decoder) - 1] = decoder_sequece(
    ...
    def execute(self, end_points):
        if self.global_feat is not None: 
            ...
            for feat_type in self.global_feat:
                if 'max' in feat_type:
                    global_feats.append(jt.max(end_points, dim=-1, keepdims=True))
                elif feat_type in ['avg', 'mean']:
                    global_feats.append(jt.mean(end_points, dim=-1, keepdims=True))
...
class PointNextSeg(nn.Module):
    ...
    def execute(self, p0, f0):
        p, f = self.encoder.execute_seg_feat(p0, f0)
...
class PointNextPartSeg(PointNextSeg):
    def __init__(self, encoder=None, decoder=None, cls_args=None, **kwargs):
        super().__init__(encoder, decoder, cls_args, **kwargs)

    def execute(self, p0, f0=None, cls0=None):
        if f0 is None:
            f0 = p0.transpose(1, 2).contiguous()
        p, f = self.encoder.execute_seg_feat(p0, f0)
        # print(p[-1].sum(-1), f[-1].sum(-1))
        if self.decoder is not None:
